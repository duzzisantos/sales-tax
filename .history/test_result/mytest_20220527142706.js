...
// We declare extra variables to support total payment as non functional varibales 
// so that the test does not return NAN with this:
      // let totalPayment3 = parseFloat(
      //   rounded1 + rounded2 + rounded3 + rounded4
      // );

let extra1 = parseFloat(importedPerf2PlusTax.toFixed(2)),
    extra2 = parseFloat(bottlePerfPlusTax.toFixed(2)),
    extra4 = parseFloat((Math.ceil(importedChoc2PlusTax * 20) / 20).toFixed(2)),
    totalPayment3 = extra1 + extra2 + pillsPlusTax + extra4


let payment3 = () => {
  return parseFloat(totalPayment3.toFixed(2));
};
