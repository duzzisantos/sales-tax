...
let totalTaxes2 = (importDuty * importedChoc) / 100 + basicPlusImport * importedPerf;
...
let importedPerf2PlusTax = (importDuty * importedPerf2) / 100 + importedPerf2;
let rounded1 = ()=>{
    return parseFloat((Math.ceil(importedPerf2PlusTax * 20) / 20).toFixed(2))
}

let bottlePerfPlusTax = (basicTax * bottlePerf) / 100 + bottlePerf;

let rounded2 = () =>{
    return parseFloat((Math.ceil(bottlePerfPlusTax * 20) / 20).toFixed(2))
}
...
let payment3 = importedPerf2PlusTax + bottlePerfPlusTax + pillsPlusTax + importedChoc2PlusTax;

let roundedPayment = () => {
  return ((Math.ceil(payment3 * 20) / 20).toFixed(2));
};
