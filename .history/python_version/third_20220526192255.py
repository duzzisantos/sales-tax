...
def basketThree():
    ...
    roundedImportedPerfume2 = format(float(importedBottleOfPerfume2PlusTax), ".2f")

    # second item
    input("1 bottle of perfume at 18.99: ")
    bottleOfPerfumePlusTax = (
        (basicTax * bottleOfPerfume) / 100) + bottleOfPerfume
    roundedBottlePerfume = format(float(bottleOfPerfumePlusTax), ".2f")
    ...
    roundedImportedChoc = format(
        (math.ceil(float(importedBoxOfChocolates2PlusTax) * 20) / 20), ".2f")
