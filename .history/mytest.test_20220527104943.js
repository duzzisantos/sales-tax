...
test("multiplies basicTax rate by musicCD price, divides by 100, and adds to musicCD price to equal 16.49", () => {
  expect(musicCDPlusTax(basicTax, musicCD).toFixed(2)).toBe(16.49);
});
...
test("total payment to equal 29.83", () => {
  expect(totalPayment(bookPlusTax, musicCDPlusTax, chocPlusTax).toFixed(2)).toBe(29.83);
});

test("total taxes to equal 1.50", () => {
  expect(
    totalTaxes(bookPlusTax, book, musicCDPlusTax, musicCD, chocPlusTax, choc).toFixed(2)
  ).toBe(1.50);
});
